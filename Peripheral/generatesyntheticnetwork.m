...
for i=1:number
    ...
    gennet = ['./benchmark -N ', int2str(N),' -k ',int2str(k), ' -maxk ', int2str(maxk), ' -beta ', int2str(beta), ' -t1 ', int2str(t1), ' -t2 ', int2str(t2), ' -on ', int2str(on), ' -om ', int2str(om), ' -muw ', num2str(muw), ' -mut ', num2str(mut)]
    system(gennet);
