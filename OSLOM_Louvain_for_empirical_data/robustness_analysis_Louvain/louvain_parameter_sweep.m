...
for gamma_list_i = 1:length(gamma_list)
    ...
    writematrix(community_affils, ...
                sprintf('%s/Louvain_results/Louvain_Rubinov_assignments_100reps_gamma%0.1f.csv', data_path, gamma_val), ...
                Delimiter=',');
end

%% 
% Looking specifically at gamma=1, let's compute the pairwise ENMI
% between each of the 100 seeds
for target_gamma = [1, 1.5]
    Louvain_at_target_gamma_res = readmatrix(sprintf('%s/Louvain_results/Louvain_Rubinov_assignments_100reps_gamma%0.1f.csv', data_path, target_gamma));
    ...
    % Save the results
    writematrix(Louvain_at_target_gamma_NMI_res, sprintf("%s/Louvain_results/NMI_results_sweep_across_seeds_%0.1f.csv", data_path, target_gamma));
